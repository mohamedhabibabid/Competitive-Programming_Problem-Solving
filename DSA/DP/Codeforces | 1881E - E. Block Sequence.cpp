...
ll solveDp(int i){
    ...
    /*  2 choix :
        choice 1 
        -> whether pick i-th as size donc solveDp(i+a[i]+1) without cost 
        but u have to check if that is valid?
        choice 2 
        -> sinn you throw i-th , add cost +1 and move to solveDp(i+1)
    */  
    ll choice1=INF;
    if(a[i]+i+1<=n) 
        choice1=solveDp(a[i]+1+i);
    
    ll choice2=solveDp(i+1)+1;
